...
截止到这一章节为止，之前所讲的所有有关线程和进程的相关内容其实都是理论性质的。在落实到实际的操作系统上，进程和线程设计实现模式又和理论是有一定差距的。简而言之，在现代linux和windows操作系统上的线程和进程的设计实现与理论是两码事。

这些线程进程的实现模式，大致可以分为如下几种

* 用户级线程

  指不需要内核支持而在用户程序层面实现的线程，其线程切换由用户程序自己控制内核切换，无需内核干涉。优点控制简单可在不支持线程的操作系统实现，缺点无法像内核级线程一样更好的运用多核CPU,一个用户级线程阻塞会引起整个进程 阻塞且无法利用好内核级别的多重处理，一次只能有一个用户线程被执行。
* 内核级线程
* 混合式线程
